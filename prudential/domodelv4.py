...
def getOptions():
    ...
    parser.add_argument('--fts',dest='fts',help='feture selection et. extraTrees, cor, randomTree. none', default='extraTrees')
...
def main():
    ...
    if args.model == 'gBoosting':
        fn = ("submissionv4_%s_gBoosting_%s_%s_%s_%s_%s.csv" % (args.fts, args.loss, str(args.minsamplessplit), str(args.lrate).replace('.','dian'),str(args.nest),str(args.maxdepth)))
...
#     test_x_uniq = indexTodata(test_x, indices)
    ...
    test_pdt = clf.predict(test_x_sel)
